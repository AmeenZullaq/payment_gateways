...
import 'package:payments/core/constants/app_endpoints.dart';
import 'package:payments/core/constants/app_keys.dart';
import 'package:payments/core/storage/secure_storage.dart';
...
class DioInterceptorWrapper implements InterceptorsWrapper {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (options.extra['isRequireToken'] == true) {
      ...
      } else if (SecureStorage.getString(AppKeys.ecretKey) != null) {
        options.headers['Authorization'] =
            'Bearer ${SecureStorage.getString(AppKeys.ecretKey)}';
      }
    }
  ...
  @override
  void onError(DioException ex, ErrorInterceptorHandler handler) async {
  ...
  Future<bool> _refreshAccessToken() async {
    try {
      final response = await dio.post(
        ...
        data: {'refreshToken': SecureStorage.getString(AppKeys.refreshToken)},
      );
